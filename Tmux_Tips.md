...
In addition, you will also need a tool capable of extracting `tar` archives. Fortunately, the Git for Windows installation includes the `tar` utility as one of the standard tools. Or your can use a GUI tool such as [7-Zip](https://www.7-zip.org/), which supports this archive type.
